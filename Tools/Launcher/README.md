...
서비스를 다시 시작하면 해당 탭의 이전 로그가 지워집니다. **현재 탭 로그 지우기** 버튼으로 실행 중에도 로그를 지울 수 있습니다.
로그를 맨 아래에서 보고 있을 때만 새 로그를 따라가며, 위로 스크롤하면 현재 위치를 유지합니다.
GPT-SoVITS와 RealtimeSTT의 콘솔 출력은 UTF-8로 설정됩니다. 변경 사항을 적용하려면 서비스를 다시 시작하세요.
