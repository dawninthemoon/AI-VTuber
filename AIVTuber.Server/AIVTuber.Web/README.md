...
기존 채팅은 건너뛰고, 이후 새 일반 텍스트 채팅을 짧은 묶음으로 답변·음성화합니다.
...
채팅은 마지막 메시지 뒤 3초 동안 더 모으되, 첫 메시지부터 최대 6초 또는 12개에 도달하면 묶음을 닫습니다.
한 묶음은 모델 호출과 음성 답변을 한 번씩 만듭니다. 웃음과 반복 반응은 대화 맥락으로 전달하고,
서로 다른 화제가 섞이면 가장 최근의 명확한 화제를 중심으로 답하도록 지시합니다.
음성이 재생되는 동안 온 채팅도 수신 시각에 따라 묶으며, 30초 넘게 대기한 묶음은 건너뜁니다.
`YouTube__BatchQuietMilliseconds`, `YouTube__BatchMaxMilliseconds`, `YouTube__BatchMaxMessages`,
`YouTube__MaxQueuedMessageAgeSeconds`로 이 값을 조정할 수 있습니다. 채팅 묶음은 로컬 처리이므로
YouTube API 요청 횟수를 늘리지 않습니다.
